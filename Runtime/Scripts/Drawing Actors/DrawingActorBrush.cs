...
{
    private static ComputeShader brushPointCompute;
    private const string brushPointComputeName = "BrushPoint";

    static DrawingActorBrush()
    {
    ...
    {
        _actorEnum = DrawingActorType.brush;
    }
    ...
    public override void UpdateStroke(Stroke stroke)
    {
        ...
        {
            int brushSize = (int)(size * Random.Range(1 - _sizeJitter, 1));
            ...
            if (brushPixRect.width > 0 && brushPixRect.height > 0)
            {
                ...
                brushPointCompute.SetTexture(0, "outputTex", stroke.strokeTexture);
                brushPointCompute.SetTexture(0, "brushAlpha", brushTexture);

                brushPointCompute.SetInts("brushRect", new int[] { brushPixRect.x, brushPixRect.y, brushPixRect.width, brushPixRect.height });
                brushPointCompute.SetInts("brushXYOffset", new int[] { brushXYOffset.x, brushXYOffset.y });
                brushPointCompute.SetFloats("brushColor", new float[] { _color.r, _color.g, _color.b, _color.a });

                brushPointCompute.Dispatch(0, HelperFunctions.xThreads(brushPixRect.width), HelperFunctions.yThreads(brushPixRect.height), 1);
    ...
    {
        if(_actorTexturePath.Length > 0)
        {
            if (System.IO.File.Exists(_actorTexturePath))
            {
                _actorTexture = AssetDatabase.LoadAssetAtPath<Texture2D>(_actorTexturePath);
            }
        }
    }
    ...
    public override void DrawGUI(Vector4 margin)
    {
        EditorGUILayout.BeginHorizontal();
            EditorGUILayout.BeginVertical();
                ...
                _actorTexture = (Texture2D)EditorGUILayout.ObjectField(_actorTexture, typeof(Texture2D), false, GUILayout.Width(120), GUILayout.Height(120));
